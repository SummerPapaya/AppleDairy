...
def choose_renderer() -> tuple[str, str | None]:
    ...
    try:
        ...
        raise SystemExit(
            "install rsvg-convert, run on macOS with sips, or pip install cairosvg"
        ) from exc
...
def render_svg(renderer: tuple[str, str | None], svg_path: Path, png_path: Path) -> None:
...
def build_frames(
    root: ET.Element,
    spec: dict,
    renderer: tuple[str, str | None],
    workspace: Path,
) -> tuple[Path, int, int, int, bool]:
    moving_ids = {item["id"] for item in [*spec["reveals"], *spec["layers"]]}
    ...
    for frame in range(frame_count):
        ...
        frame_image = flatten_to_chroma(canvas, transparent_color, alpha_threshold)
